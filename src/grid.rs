...
impl<T, H: BuildHasher, const S: usize> ExGridSparse<T, S, H> {
  ...
  pub fn entry(&mut self, pos: impl Into<GlobalPos>) -> ExGridSparseEntry<'_, T, S> {
    ExGridSparseEntry::new(pos, &mut self.chunks)
  }
}
...
impl<'a, T, const S: usize> ExGridSparseEntry<'a, T, S> {
  fn new<H>(pos: impl Into<GlobalPos>, chunks: &'a mut HashMap<ChunkPos, ChunkSparse<T, S>, H>) -> Self
  where H: BuildHasher {
    let (chunk, local) = decompose::<S>(pos.into());
    ExGridSparseEntry { entry: chunks.entry(chunk), pos: local }
  }
...
impl<T, H: BuildHasher, const S: usize> ExGrid<T, S, H> {
  ...
  pub fn entry(&mut self, pos: impl Into<GlobalPos>) -> ExGridEntry<'_, T, S> {
    ExGridEntry::new(pos, &mut self.chunks)
  }
}
...
impl<'a, T, const S: usize> ExGridEntry<'a, T, S> {
  fn new<H>(pos: impl Into<GlobalPos>, chunks: &'a mut HashMap<ChunkPos, Chunk<T, S>, H>) -> Self
  where H: BuildHasher {
    let (chunk, local) = decompose::<S>(pos.into());
    ExGridEntry { entry: chunks.entry(chunk), pos: local }
  }
...
pub fn decompose<const S: usize>(pos: GlobalPos) -> (ChunkPos, LocalPos) {
  assert!(S > 0 && S <= i64::MAX as usize, "cannot represent chunks of size {S}");
...
pub fn compose<const S: usize>(chunk: ChunkPos, local: LocalPos) -> GlobalPos {
  assert!(S > 0 && S <= i64::MAX as usize, "cannot represent chunks of size {S}");
  let chunk = Vector2::from_array(chunk).cast::<i64>();
  let local = Vector2::from_array(local).cast::<i64>();
  Vector2::into_array(chunk * S as i64 + local)
}
