...
impl<T, const S: usize> ChunkSparse<T, S> {
  ...
  pub const fn as_array(&self) -> &[[Option<T>; S]; S] {
    &self.inner.inner
  }

  pub const fn as_array_mut(&mut self) -> &mut [[Option<T>; S]; S] {
    &mut self.inner.inner
  }

  pub fn into_array(self) -> [[Option<T>; S]; S] {
    self.inner.inner
  }
...
impl<T> ChunkSparse<T, 2> {
  pub(crate) fn init_corners<F>(pos: impl Into<[f32; 2]>, f: F) -> Self
...
impl<T, const S: usize> AsRef<[[Option<T>; S]; S]> for ChunkSparse<T, S> {
  fn as_ref(&self) -> &[[Option<T>; S]; S] {
    self.as_array()
  }
}

impl<T, const S: usize> AsMut<[[Option<T>; S]; S]> for ChunkSparse<T, S> {
  fn as_mut(&mut self) -> &mut [[Option<T>; S]; S] {
    self.as_array_mut()
  }
}
...
impl<T, const S: usize> Chunk<T, S> {
  ...
  where F: FnMut(T) -> Option<U> {
    ...
    self.inner.try_map_opt(|slice| slice.try_map_opt(&mut f))
  ...
  where F: FnMut(T) -> Result<U, E> {
    use crate::array_util::ArrayExt;
    self.inner.try_map_res(|slice| slice.try_map_res(&mut f))
      .map(|inner| Chunk { inner })
  }
  ...
  pub const fn as_array(&self) -> &[[T; S]; S] {
    &self.inner
  }

  pub const fn as_array_mut(&mut self) -> &mut [[T; S]; S] {
    &mut self.inner
  }

  pub fn into_array(self) -> [[T; S]; S] {
    self.inner
  }
...
impl<T, const S: usize> AsRef<[[T; S]; S]> for Chunk<T, S> {
  fn as_ref(&self) -> &[[T; S]; S] {
    self.as_array()
  }
}

impl<T, const S: usize> AsMut<[[T; S]; S]> for Chunk<T, S> {
  fn as_mut(&mut self) -> &mut [[T; S]; S] {
    self.as_array_mut()
  }
}
...
mod tests {
  ...
  fn test_lerp() {
    ...
    assert_eq!(chunk.sample([0.0, 0.0]), 25.0, "at [0, 0]");
    assert_eq!(chunk.sample([1.0, 0.0]), 10.0, "at [1, 0]");
    assert_eq!(chunk.sample([0.0, 1.0]), 13.7, "at [0, 1]");
    assert_eq!(chunk.sample([1.0, 1.0]), 40.2, "at [1, 1]");
  }
}
